...
# Huggingface datasets/tokenizers are used inside build_tokenized_dataset
...
def compute_validation_metrics(model, val_dataloader, loss_fn, tokenizer_src, tokenizer_tgt, max_len, device, pad_id, vocab_size, max_samples=None):
    """Return (val_loss, val_ppl, val_acc). If max_samples is set, use only that many samples (for periodic step metrics)."""
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_non_pad = 0
    ...
    if max_samples is not None:
        print(f"Computing validation metrics (subset: up to {max_samples} samples)...")
    else:
        print("Computing validation metrics (full val set)...")

    with torch.no_grad():
        for batch in val_dataloader:
            if max_samples is not None and num_val_samples >= max_samples:
                break
            ...
            if max_samples is not None and num_val_samples + b > max_samples:
                b = max_samples - num_val_samples
                encoder_input, decoder_input = encoder_input[:b], decoder_input[:b]
                encoder_mask, decoder_mask = encoder_mask[:b], decoder_mask[:b]
                label = label[:b]
            num_val_samples += b
    ...
    print(f"  -> val_loss={total_loss / num_val_samples if num_val_samples else 0:.4f} (n={num_val_samples})")
